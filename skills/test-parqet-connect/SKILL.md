...
- Keep client secrets server-side; never embed them in browser code.
- Never log or expose access/refresh tokens; store them server-side and protect with least-privilege scopes.
- If you need a standard Parqet OAuth button, use the snippet in `references/oauth-button.md` and generate the authorize URL server-side or via OAuth metadata.
